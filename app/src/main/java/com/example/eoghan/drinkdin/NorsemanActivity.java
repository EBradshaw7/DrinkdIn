...
public class NorsemanActivity extends AppCompatActivity implements OnMapReadyCallback, View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_norseman);
        // Obtain the SupportMapFragment and get notified when the map is ready to be used.
        SupportMapFragment mapFragment = (SupportMapFragment) getSupportFragmentManager()
                .findFragmentById(R.id.map);
        mapFragment.getMapAsync(this);

        getSupportActionBar().setTitle("The Norseman");


        //rating bar
        norseRating = (RatingBar) findViewById(R.id.norsemanRating);
        norseRating.setOnRatingBarChangeListener(new RatingBar.OnRatingBarChangeListener() {
            @Override
        ...
        Query readRatingAvg = databaseReference.child("ratings").child("norseman");
        readRatingAvg.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                //get data from snapshot ensure it is not null
                if (dataSnapshot.child("norseman").child("averageRating").getValue() == null) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.homeIcon:
                Intent i = new Intent(NorsemanActivity.this, MainActivity.class);
                startActivity(i);
                return true;

            case R.id.userIcon:
                Intent j = new Intent(NorsemanActivity.this, LoginActivity.class);
                startActivity(j);
                return true;
        ...
        if (ratingStr != null) {
            ...
            if (user != null) {
                ...
                databaseReference.child("checkin").child(
                        user.getUid()).child("Norseman").child("Rating").setValue(ratingStr);

                //add to the number of ratings stored once the rating is submitted
                Query RetrieveRatingCount = databaseReference.child("ratings").child("norseman");
                RetrieveRatingCount.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot postSnapshot) {
                        ...
                        if (!data.equals(null)) {
                            ...
                            databaseReference.child("ratings").child("norseman").child("numRating").setValue(ratingCounter);
                ...
                Query addRatingTotal = databaseReference.child("ratings").child("norseman");
                addRatingTotal.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot postSnapshot2) {
                        ...
                        databaseReference.child("ratings").child("norseman").child("totalRating").setValue(newRatingTotal);
                ...
                Query calcAverage = databaseReference.child("ratings").child("norseman");
                calcAverage.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot postSnapshot3) {
                        ...
                        //post it to database
                        databaseReference.child("ratings").child("norseman").child("averageRating").setValue(averageRatingStr);
    ...
    private void AddToList() {
        ...
        if (user != null) {
            ...
            databaseReference.child("checkin").child(user.getUid()).child("Norseman").child("timeStamp").setValue(date);

            //set this pub as last check in, over writing previous
            databaseReference.child("users").child(user.getUid()).child("lascheckin").setValue("The Norseman");
