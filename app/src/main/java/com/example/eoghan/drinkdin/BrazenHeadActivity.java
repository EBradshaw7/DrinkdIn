...
public class BrazenHeadActivity extends AppCompatActivity implements OnMapReadyCallback, View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_brazen_head);
        // Obtain the SupportMapFragment and get notified when the map is ready to be used.
        SupportMapFragment mapFragment = (SupportMapFragment) getSupportFragmentManager()
                .findFragmentById(R.id.map);
        mapFragment.getMapAsync(this);

        getSupportActionBar().setTitle("The Brazen Head");


        //rating bar
        bhRating = (RatingBar) findViewById(R.id.brazenRating);
        bhRating.setOnRatingBarChangeListener(new RatingBar.OnRatingBarChangeListener() {
            @Override
        ...
        Query readRatingAvg = databaseReference.child("ratings").child("brazenhead");
        readRatingAvg.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                //get data from snapshot ensure it is not null
                if (dataSnapshot.child("brazenhead").child("averageRating").getValue() == null) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.homeIcon:
                Intent i = new Intent(BrazenHeadActivity.this, MainActivity.class);
                startActivity(i);
                return true;
        ...
        if (ratingStr != null) {
            ...
            if (user != null) {
                ...
                databaseReference.child("checkin").child(
                        user.getUid()).child("BrazenHead").child("Rating").setValue(ratingStr);

                //add to the number of ratings stored once the rating is submitted
                Query RetrieveRatingCount = databaseReference.child("ratings").child("brazenhead");
                RetrieveRatingCount.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot postSnapshot) {
                        ...
                        if (!data.equals(null)) {
                            ...
                            databaseReference.child("ratings").child("brazenhead").child("numRating").setValue(ratingCounter);
                ...
                Query addRatingTotal = databaseReference.child("ratings").child("brazenhead");
                addRatingTotal.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot postSnapshot2) {
                        ...
                        databaseReference.child("ratings").child("brazenhead").child("totalRating").setValue(newRatingTotal);
                ...
                Query calcAverage = databaseReference.child("ratings").child("brazenhead");
                calcAverage.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot postSnapshot3) {
                        ...
                        //post it to database
                        databaseReference.child("ratings").child("brazenhead").child("averageRating").setValue(averageRatingStr);
    ...
    private void AddToList() {
        ...
        if (user != null) {
            ...
            databaseReference.child("checkin").child(user.getUid()).child("BrazenHead").child("timeStamp").setValue(date);

            //set this pub as last check in, over writing previous
            databaseReference.child("users").child(user.getUid()).child("lascheckin").setValue("Brazen Head");
