...
public class TempleBarActivity extends AppCompatActivity implements OnMapReadyCallback, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_temple_bar);
        // Obtain the SupportMapFragment and get notified when the map is ready to be used.
        SupportMapFragment mapFragment = (SupportMapFragment) getSupportFragmentManager()
                .findFragmentById(R.id.map);
        mapFragment.getMapAsync(this);

        getSupportActionBar().setTitle("The Temple Bar");


        //rating bar
        tbRating = (RatingBar) findViewById(R.id.templeRating);
        tbRating.setOnRatingBarChangeListener(new RatingBar.OnRatingBarChangeListener() {
            @Override
        ...
        Query readRatingAvg = databaseReference.child("ratings").child("templebar");
        readRatingAvg.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                //get data from snapshot ensure it is not null
                if (dataSnapshot.child("templebar").child("averageRating").getValue() == null) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.homeIcon:
                Intent i = new Intent(TempleBarActivity.this, MainActivity.class);
                startActivity(i);
                return true;
        ...
        if (ratingStr != null) {
            ...
            if (user != null) {
                ...
                databaseReference.child("checkin").child(
                        user.getUid()).child("TempleBar").child("Rating").setValue(ratingStr);

                //add to the number of ratings stored once the rating is submitted
                Query RetrieveRatingCount = databaseReference.child("ratings").child("templebar");
                RetrieveRatingCount.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot postSnapshot) {
                        ...
                        if (!data.equals(null)) {
                            ...
                            databaseReference.child("ratings").child("templebar").child("numRating").setValue(ratingCounter);
                ...
                Query addRatingTotal = databaseReference.child("ratings").child("templebar");
                addRatingTotal.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot postSnapshot2) {
                        ...
                        databaseReference.child("ratings").child("templebar").child("totalRating").setValue(newRatingTotal);
                ...
                Query calcAverage = databaseReference.child("ratings").child("templebar");
                calcAverage.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot postSnapshot3) {
                        ...
                        //post it to database
                        databaseReference.child("ratings").child("templebar").child("averageRating").setValue(averageRatingStr);
    ...
    private void AddToList() {
        ...
        if (user != null) {
            ...
            databaseReference.child("checkin").child(user.getUid()).child("TempleBar").child("timeStamp").setValue(date);

            //set this pub as last check in, over writing previous
            databaseReference.child("users").child(user.getUid()).child("lascheckin").setValue("The Temple Bar");
