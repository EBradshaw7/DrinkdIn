...
public class DingleActivity extends AppCompatActivity implements OnMapReadyCallback, View.OnClickListener{
    ...
    //layout elements
    private RatingBar dingleRating;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_dingle);
        // Obtain the SupportMapFragment and get notified when the map is ready to be used.
        SupportMapFragment mapFragment = (SupportMapFragment) getSupportFragmentManager()
                .findFragmentById(R.id.map);
        mapFragment.getMapAsync(this);

        getSupportActionBar().setTitle("The Dingle Bar");


        //rating bar
        dingleRating = (RatingBar) findViewById(R.id.dingleRating);
        dingleRating.setOnRatingBarChangeListener(new RatingBar.OnRatingBarChangeListener() {
            @Override
        ...
        Query readRatingAvg = databaseReference.child("ratings").child("dingle");
        readRatingAvg.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                //get data from snapshot ensure it is not null
                if (dataSnapshot.child("dingle").child("averageRating").getValue() == null) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.homeIcon:
                Intent i = new Intent(DingleActivity.this, MainActivity.class);
                startActivity(i);
                return true;
        ...
        if (ratingStr != null) {
            ...
            if (user != null) {
                ...
                databaseReference.child("checkin").child(
                        user.getUid()).child("Dingle").child("Rating").setValue(ratingStr);

                //add to the number of ratings stored once the rating is submitted
                Query RetrieveRatingCount = databaseReference.child("ratings").child("dingle");
                RetrieveRatingCount.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot postSnapshot) {
                        ...
                        if (!data.equals(null)) {
                            ...
                            databaseReference.child("ratings").child("dingle").child("numRating").setValue(ratingCounter);
                ...
                Query addRatingTotal = databaseReference.child("ratings").child("dingle");
                addRatingTotal.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot postSnapshot2) {
                        ...
                        databaseReference.child("ratings").child("dingle").child("totalRating").setValue(newRatingTotal);
                ...
                Query calcAverage = databaseReference.child("ratings").child("dingle");
                calcAverage.addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(DataSnapshot postSnapshot3) {
                        ...
                        //post it to database
                        databaseReference.child("ratings").child("dingle").child("averageRating").setValue(averageRatingStr);
    ...
    private void AddToList() {
        ...
        if (user != null) {
            ...
            databaseReference.child("checkin").child(user.getUid()).child("Dingle").child("timeStamp").setValue(date);

            //set this pub as last check in, over writing previous
            databaseReference.child("users").child(user.getUid()).child("lascheckin").setValue("Dingle Bar");
